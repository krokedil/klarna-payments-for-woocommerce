...
final class Redactor
{
    ...
    public static function withDefaultPatterns(): self
    {
        return new self([], [
            ...
            '/\b(NGROK_AUTHTOKEN|KLARNA_TEST_SECRET_[A-Z]{2}|WORDPRESS_ADMIN_PASSWORD)\s*=\s*\S+/'
                => '$1=' . self::MASK,
        ]);
    }
    ...
    /**
     * Registers the base64 of `username:password`, which is the form KP
    ...
    public function withBasicAuthPair(string $username, string $password, string $label): self
    {
        ...
        // KP decodes HTML entities out of the secret before encoding, so cover both spellings.
