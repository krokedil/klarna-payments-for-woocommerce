 ...
 * credentials without a .env file. Regional KLARNA_TEST_SECRET_<CC> keys are discovered
...
final class SecretRegistry
{
    ...
    public const SECRET_KEYS = [
        'KLARNA_TEST_SECRET_SE',
        'NGROK_AUTHTOKEN',
        'WORDPRESS_ADMIN_PASSWORD',
    ];

    /**
     * Merchant-id / secret pairs, as `[midKey, secretKey]`. KP sends these
     * base64'd together, so the pair needs registering as well as the parts.
     *
     * @var array<int, array{0: string, 1: string}>
     */
    private const BASIC_AUTH_PAIRS = [
        ['KLARNA_TEST_MID_SE', 'KLARNA_TEST_SECRET_SE'],
    ];
    ...
    public static function fromEnvironment(?string $envFile = null, ?array $env = null): Redactor
    {
        ...
        // Any regional Klarna secret present in the file, plus the fixed list.
        $keys = self::SECRET_KEYS;
        foreach (array_keys($values) as $key) {
            if (preg_match('/^KLARNA_TEST_SECRET_[A-Z]{2}$/', $key) === 1) {
                $keys[] = $key;
            }
        }
