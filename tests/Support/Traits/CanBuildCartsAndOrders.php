...
trait CanBuildCartsAndOrders {
	...
	protected function haveOrder( array $args = [] ): \WC_Order {
		...
		if ( false !== $args['klarna'] ) {
			$klarna = is_array( $args['klarna'] ) ? $args['klarna'] : [];

			$this->applyKlarnaMeta(
				$order,
				$klarna['order_id'] ?? 'klarna-order-123',
				$klarna['country'] ?? 'SE'
			);
		}
	...
	/**
	 * A saved order that went through Klarna: one 100.00 simple product, Swedish
	 * billing address, and the Klarna order meta. Needs `CanManageProducts`
	 * composed alongside (as `IntegrationTestCase` does).
	 */
	protected function haveKlarnaOrder( array $args = [] ): \WC_Order {
	...
	/** A paid Klarna order in processing (2 x 100.00), ready to be captured. */
	protected function haveCapturableKlarnaOrder( array $args = [] ): \WC_Order {
		return $this->haveKlarnaOrder(
			array_merge(
				[
	...
	/** Stamps an order with the meta a completed Klarna purchase leaves behind. */
	protected function markAsKlarnaOrder( \WC_Order $order, string $klarna_order_id = 'klarna-order-123', string $country = 'SE' ): \WC_Order {
		$this->applyKlarnaMeta( $order, $klarna_order_id, $country );
		$order->save();

		return $order;
	}

	/** Writes the Klarna meta onto an order, without saving. */
	private function applyKlarnaMeta( \WC_Order $order, string $klarna_order_id, string $country ): void {
