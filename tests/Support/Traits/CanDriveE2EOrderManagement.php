...
/**
 * The admin half of an order's life: drive the WooCommerce order screen and the Klarna
 * Order Management metabox, then read back what Klarna and WooCommerce made of it.
 *
...
trait CanDriveE2EOrderManagement {
	...
	/** The KOM metabox, which is only on the screen once the order page has rendered. */
	private const KOM_METABOX = '#klarna-om';

	/** How long a save that talks to Klarna may take, in seconds. */
	private const KOM_SAVE_TIMEOUT = 90;

	/** Whether this test has already moved the browser to wp-admin and logged in. */
	private bool $inKlarnaAdmin = false;

	/** Merges settings over the ones already in the database, keyed by setting name. */
	public function haveKlarnaSettingsInDatabase( array $overrides ): void {
		$settings = $this->grabOptionFromDatabase( 'woocommerce_klarna_payments_settings' );
	...
	/** Klarna's own status for the order, as the metabox just read it back. */
	public function seeKlarnaOrderStatusIs( string $status ): void {
		$actual = trim(
			(string) $this->grabTextFrom(
				'//div[@id="klarna-om"]//h4[contains(.,"Klarna order status")]/following-sibling::span[1]'
			)
		);

		Assert::assertSame( $status, $actual, "Klarna says the order is '{$actual}', expected '{$status}'." );
	}

	/** Moves the order to a status and waits out whatever it makes KP send. */
	public function changeOrderStatusTo( string $status ): void {
		$this->selectOption( '#order_status', "wc-{$status}" );
		$this->saveOrderScreen();
	}

	/** Applies a KOM metabox action, which only renders when its kom_auto_* setting is off. */
	public function applyKlarnaOrderAction( string $action ): void {
	...
	public function refundOrderViaKlarna( string $amount ): void {
		...
		$this->executeJS(
			'window.kpRefunding = true; window.kpRefundError = null;'
			. ' window.confirm = () => true;'
			. ' window.alert = message => { window.kpRefundError = message; };'
		);
		...
		$this->waitForJS(
			'return window.kpRefunding === undefined || !!window.kpRefundError;',
			self::KOM_SAVE_TIMEOUT
		);

		$error = $this->executeJS( 'return window.kpRefundError || null;' );
		if ( ! empty( $error ) ) {
	 ...
	 * gateway errors, so the count surviving is Klarna having taken the body.
	 */
	 ...
	 * the driver's budget: saving one is a Klarna round trip before the response even
