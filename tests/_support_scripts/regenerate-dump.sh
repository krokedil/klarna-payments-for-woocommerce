...
#
# Pre-conditions:
#   - `composer install` has run (tests/_wordpress exists with WC + KP
#     symlinked in).
#   - `npm install && npm run build` has run so KP's frontend assets are
...
echo "==> Activating plugins (WooCommerce first, then KP)..."
...
echo "==> Setting placeholder Klarna Payments settings (enabled, test mode, SE)..."
# Credentials are not committed to the dump on purpose, they're injected
# at request time by tests/_mu-plugins/02-klarna-test-credentials.php from
# KLARNA_TEST_MID_SE / KLARNA_TEST_SECRET_SE. The placeholders here just
# guarantee the option row exists so the filter has something to merge into.
${WP} option update woocommerce_klarna_payments_settings \
'{"enabled":"yes","testmode":"yes", "logging":"yes", "customer_type":"b2c","test_merchant_id_se":"placeholder","test_shared_secret_se":"placeholder"}' \
--format=json
