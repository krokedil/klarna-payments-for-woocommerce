 ...
 * subscription test green for the wrong reason. Calls the globals KP calls,
 * not the registry, because the wiring between the two is part of what can break.
 *
...
class FakeSubscriptionsTest extends IntegrationTestCase {
	...
	/** The order types KP asks about in KP_Subscription::order_has_subscription(). */
	private const KP_ORDER_TYPES = [ 'parent', 'resubscribe', 'switch', 'renewal' ];
	...
	public function test_a_test_that_registers_nothing_sees_a_store_without_subscriptions(): void {
		...
		$this->assertFalse( wcs_order_contains_subscription( $order, self::KP_ORDER_TYPES ) );
	...
	public function test_a_renewal_order_finds_its_subscription_only_when_the_order_type_allows_it(): void {
		...
		$this->assertSame(
			...
			'KP asks for "any" when saving a recurring token, and expects the renewal to resolve.'
		);
		$this->assertSame(
			[ $subscription->get_id() ],
