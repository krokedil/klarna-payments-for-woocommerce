...
jQuery( function ( $ ) {
	...
	var klarna_payments = {
		...
		load: function () {
			...
			log( klarna_payments_container_selector_id )
		...
		authorizeKlarnaOrder: function ( order_id, order_key ) {
			klarna_payments.authorize().done( function ( response ) {
				if ( "authorization_token" in response ) {
					...
					klarna_payments.logToFile( `(Order ${ order_id }): No authorization_token in response. Reason: ${ reason }.`)
