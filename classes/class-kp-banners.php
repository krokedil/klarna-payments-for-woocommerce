...
if ( ! class_exists( 'KP_Banners' ) ) {
	...
	class KP_Banners {
		...
		public function klarna_banner() {
			...
			$country_set = false;
			foreach ( KP_Form_Fields::$kp_form_auto_countries as $country_name => $country_code ) {
				$merchant_id   = 'merchant_id_' . $country_code;
				$shared_secret = 'shared_secret_' . $country_code;
