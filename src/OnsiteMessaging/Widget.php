...
class Widget extends \WP_Widget
{
    ...
    public function __construct($onsite_messaging)
    {
        ...
        parent::__construct(
            'klarna_osm',
            // Base ID.
            \__('Klarna On-Site Messaging', 'klarna-onsite-messaging-for-woocommerce'),
            // Name.
            array('description' => \__('Displays a Klarna banner in your store.', 'klarna-onsite-messaging-for-woocommerce'))
        );
    }
    /**
    ...
    public function widget($args, $instance)
    {
        ...
        $instance = \wp_parse_args($instance, array('key' => 'homepage-promotion-wide', 'theme' => '', 'purchase-amount' => ''));
        ...
        echo \esc_attr(\Krokedil\Klarna\OnsiteMessaging\Utility::print_placement($instance));
    ...
    public function form($instance)
    {
        ...
        ?>" name="<?php 
        echo \esc_html($this->get_field_name('title'));
        ?>" type="text" value="<?php 
        echo \esc_attr($title);
        ?>" />
		</p>
        ...
        ?>" type="text"
				value="<?php 
        echo \esc_attr($data_key);
        ?>" />
		</p>
		<p>
			<label for="<?php 
        echo \esc_attr($this->get_field_id('theme'));
        ?>"><?php 
        \esc_html_e('Placement Theme:');
        ?></label>
			<select class="widefat" id="<?php 
        echo \esc_attr($this->get_field_id('theme'));
        ?>"
					name="<?php 
        echo \esc_attr($this->get_field_name('theme'));
        ?>">
				<option value="default" <?php 
        \selected($data_theme, 'default');
        ?>>Default</option>
				<option value="dark" <?php 
        \selected($data_theme, 'dark');
        ?>>Dark</option>
			</select>
		</p>
		<?php 
