...
PLUGIN_FILE="klarna-payments-for-woocommerce.php" # Main plugin file (e.g., "my-gateway.php")
README_FILE="readme.txt"                          # WordPress readme file
CHANGELOG_FILE="changelog.txt"                    # Standalone changelog file (optional, leave empty if not used)
CHANGELOG_DIR=".changelogs"                       # Directory for changelog entries
KERNL_VERSION_FILE="kernl.version"                # Kernl version file (optional, leave empty if not used)
KERNL_CHANGELOG_FILE="changelog.json"             # Kernl changelog file (optional, leave empty if not used)
...
VERSION_CONSTANT_NAME="WC_KLARNA_PAYMENTS_VERSION"
...
REQUIRE_CHANGELOG_FILE="true"       # Require standalone changelog.txt file
...
MONITORED_FILES=(readme.txt composer.json composer.lock composer-dependencies.json composer-dependencies.lock)
