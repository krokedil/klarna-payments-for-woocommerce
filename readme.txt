...
* Fix           - The order's shipping address is now sent to Klarna when paying for an existing order, instead of falling back to the billing address.
* Fix           - Fixed a JavaScript error ("Uncaught TypeError: Cannot read properties of null (reading 'shadowRoot')") that could occur on pages where the KEC script was enqueued but the KEC button markup was not rendered.

= 2026-08-03    - version 4.12.0 =
* Feature       - On-site messaging functionality is now built directly into the plugin, eliminating the need for external code packages.
* Feature       - Express Checkout functionality is now built directly into the plugin, eliminating the need for external code packages.
* Feature       - Sign in with Klarna functionality is now built directly into the plugin, eliminating the need for external code packages.
* Fix           - The Two-Step Klarna Express Checkout flow now works as expected when the redirect checkout flow is used.
* Fix           - Fixed an issue where Klarna orders could fail to complete on stores using multiple currencies, when the shopper's currency was different from the store's default currency.
* Fix           - Refunds containing zero-value order lines, such as free products or free shipping, no longer fail with a division-by-zero error.
...
* Fix           - wp_debug_backtrace_summary is now used to build the stacktrace included in the plugin logs. This resolves an issue where a fatal error could occur in the order management metabox in some environments.
...
* Change        - Changed minimum supported PHP version to 8.0.
...
= 2026-04-09    - version 4.10.1 =
* Fix           - Sanitize the client id before using or saving it to the settings.
...
= 2026-03-23    - version 4.7.1 =
* Enhancement   - Introduced the `kp_wc_api_request_body_args` filter to provide more specific API request customization and removed duplicate filtering previously applied via `kp_wc_api_request_args`.
* Tweak         - Added a group name to the scheduled action `kp_wc_authorization` to improve filtering of related scheduled actions listed on the order admin page.
